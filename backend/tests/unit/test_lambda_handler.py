...
from functions.invoice_processor.lambda_function import (
    lambda_handler,
    _process_record,
    _infer_content_type,
    _get_request_id,
...
def _make_s3_event(
    ...
    key: str = "invoices/incoming/abc-123-def/invoice.pdf",
    size: int = 50_000,
...
class TestLambdaHandler:
    ...
    @patch("functions.invoice_processor.lambda_function.process_invoice")
    def test_processes_valid_single_record(self, mock_processor):
        ...
        mock_processor.assert_called_once_with(
            ...
            s3_key="invoices/incoming/abc-123-def/invoice.pdf",
        )
    ...
    def test_processes_multiple_records(self, mock_processor):
        ...
        event = {
            "Records": [
                _make_record(key="invoices/incoming/id-1/file1.pdf"),
                _make_record(key="invoices/incoming/id-2/file2.png"),
            ]
        }
        result = lambda_handler(event, MockContext())
    ...
    def test_url_encoded_keys_are_decoded(self, mock_processor):
        ...
        event = _make_s3_event(key="invoices/incoming/abc-123/my+invoice+file.pdf")
        lambda_handler(event, MockContext())

        mock_processor.assert_called_once_with(
            bucket="intelliprocess-ai-documents",
            s3_key="invoices/incoming/abc-123/my invoice file.pdf",
        )

    @patch("functions.invoice_processor.lambda_function.process_invoice")
    def test_percent_encoded_keys_are_decoded(self, mock_processor):
        """S3 keys with %XX encoding are properly decoded."""
        event = _make_s3_event(key="invoices/incoming/abc-123/invoice%20%282%29.pdf")
        lambda_handler(event, MockContext())

        mock_processor.assert_called_once_with(
            bucket="intelliprocess-ai-documents",
            s3_key="invoices/incoming/abc-123/invoice (2).pdf",
        )
...
class TestProcessRecordValidation:
    ...
    def test_valid_pdf_is_processed(self, mock_processor):
        ...
        record = _make_record(key="invoices/incoming/doc-id-1/bill.pdf", size=1024)
    ...
    def test_valid_png_is_processed(self, mock_processor):
        ...
        record = _make_record(key="invoices/incoming/doc-id-1/scan.png", size=2048)
        result = _process_record(record)
        assert result["status"] == "processed"
    ...
    def test_valid_jpeg_is_processed(self, mock_processor):
        ...
        record = _make_record(key="invoices/incoming/doc-id-1/photo.jpeg", size=3000)
        result = _process_record(record)
        assert result["status"] == "processed"
    ...
    def test_valid_jpg_is_processed(self, mock_processor):
        ...
        record = _make_record(key="invoices/incoming/doc-id-1/photo.jpg", size=3000)
        result = _process_record(record)
        assert result["status"] == "processed"

    def test_non_incoming_stage_is_skipped(self):
        """Objects already in processed/ or failed/ are skipped (no reprocessing)."""
        for stage in ("processed", "failed"):
            record = _make_record(key=f"invoices/{stage}/doc-id-1/bill.pdf", size=1024)
            result = _process_record(record)
    ...
    def test_invalid_key_structure_is_skipped(self):
        """S3 key with fewer than 4 parts (missing stage or id) is skipped."""
        record = _make_record(key="invoices/incoming/only-three-parts")
        result = _process_record(record)
        assert result["status"] == "skipped"
    ...
    def test_non_invoice_prefix_is_skipped(self):
        ...
        record = _make_record(key="records/incoming/doc-id/file.pdf")
        result = _process_record(record)
        assert result["status"] == "skipped"
    ...
    def test_recognized_but_unsupported_type_is_rejected(self, mock_processor):
        ...
        import functions.invoice_processor.lambda_function as handler_module
        ...
        try:
            record = _make_record(key="invoices/incoming/doc-id/contract.docx")
            result = _process_record(record)
            assert result["status"] == "failed"
    ...
    def test_unknown_extension_is_still_processed(self, mock_processor):
        ...
        record = _make_record(key="invoices/incoming/doc-id-1/some-file")
        result = _process_record(record)
...
class TestProcessRecordEdgeCases:
    ...
    def test_deep_key_path_is_processed(self, mock_processor):
        ...
        record = _make_record(key="invoices/incoming/doc-id/subfolder/deep/file.pdf")
        result = _process_record(record)
        assert result["status"] == "processed"
    ...
    def test_key_with_special_characters(self, mock_processor):
        ...
        record = _make_record(key="invoices/incoming/doc-id-with-dashes/file (1).pdf")
        result = _process_record(record)
        assert result["status"] == "processed"
